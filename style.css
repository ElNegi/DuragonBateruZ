...
(function(){
  ...
  function jugadorLobbyInicial(user){
    return {
      ...
      hp: null, maxHp: null, gold: 0, baseDamage: 2, diceCount: 1, specialization: user.specialization || null,
      activeDeck: [], bench: []
    };
  }
  ...
  function renderLobby(room){
    ...
    if(listEl){
      ...
      (room.playerOrder || []).forEach(function(uid){
        ...
        var specLabel=p.specialization;
        if(specLabel && window.Engagement && p.race){ var sp=(window.Engagement.getSpecializations(p.race)||[]).find(function(x){return x.id===p.specialization;}); if(sp)specLabel=sp.name; }
        row.innerHTML =
          '<span class="mp-player-avatar">' + (p.race ? raceEmoji(p.race) : '❔') + '</span>' +
          '<span class="mp-player-name">' + p.name + (uid === room.hostUid ? ' (Anfitrión)' : '') + '</span>' +
          '<span class="mp-player-state">' + (p.race ? p.race : 'Eligiendo raza…') + (specLabel ? ' · '+specLabel : '') + (p.ready ? ' · Listo ✅' : '') + '</span>';
    ...
    if(raceContainer){
      ...
      if(meRoom.race && window.Engagement){
        var title=document.createElement('div'); title.className='mp-specialization-title'; title.textContent='🌟 Especialización'; raceContainer.appendChild(title);
        window.Engagement.getSpecializations(meRoom.race).forEach(function(sp){
          var b=document.createElement('button'); b.type='button'; b.className='race-card mp-race-card mp-spec-card'+(meRoom.specialization===sp.id?' selected':'');
          b.innerHTML='<div class="race-avatar">'+sp.icon+'</div><div class="race-name">'+sp.name+'</div><div class="race-desc">'+sp.desc+'</div>';
          b.addEventListener('click',function(){elegirEspecializacionLobby(sp.id);}); raceContainer.appendChild(b);
        });
      }
    }
    ...
    if(readyBtn){
      readyBtn.disabled = !meRoom.race || (window.Engagement && !meRoom.specialization);
    ...
    if(statusEl){
      ...
      else if(!todosListos) statusEl.textContent = 'Elige raza, especialización y pulsa "Listo" para continuar.';
  ...
  async function elegirRazaLobby(race){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        if(!p) return;
        if(p.race!==race) p.specialization=null;
        p.race = race;
        p.ready = false;
  ...
  async function elegirEspecializacionLobby(specId){
    var roomRef=doc(db,'rooms',state.roomCode);
    try{
      await runTransaction(db,async function(tx){
        var snap=await tx.get(roomRef), room=snap.data(), p=room.players[miUid()];
        if(!p||!p.race||!window.Engagement)return;
        var valid=window.Engagement.getSpecializations(p.race).some(function(sp){return sp.id===specId;});
        if(!valid)return;
        p.specialization=specId; p.ready=false; room.players[miUid()]=p;
        tx.update(roomRef,{players:room.players,updatedAt:serverTimestamp()});
      });
    }catch(err){console.error(err);}
  }
  ...
  async function alternarListo(){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        var room = snap.data();
        var p = room.players[miUid()];
        if(!p || !p.race || (window.Engagement && !p.specialization)) return;
        p.ready = !p.ready;
        room.players[miUid()] = p;
        tx.update(roomRef, { players: room.players, updatedAt: serverTimestamp() });
      });
  ...
  function construirMazoInicial(race){
    ...
    return {
      activeDeck: pool.slice(0,4).concat(neutral.slice(0,4)).map(clone),
      bench: pool.slice(4,6).concat(neutral.slice(4,6)).map(clone)
    };
  }
  ...
  async function iniciarExpedicionMP(){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        var uids = Object.keys(room.players || {});
        if(uids.length !== 2 || !uids.every(function(u){ return room.players[u].race && room.players[u].ready && (!window.Engagement || room.players[u].specialization); })) return;
  ...
  function renderMapaMP(room){
    ...
    var activeNode=nodes.find(function(n){return n.estado==='disponible'||n.estado==='in-progress';}) || nodes[room.currentNodeIndex||0];
    var saga=window.Engagement&&activeNode?window.Engagement.getSaga(activeNode.acto):null;
    var mpHeading=document.getElementById('mpMapHeading'); if(mpHeading)mpHeading.textContent=saga?(saga.icon+' Acto '+activeNode.acto+' · '+saga.name):'Sendero Cooperativo';
    var completedCount = nodes.filter(function(n){ return n.estado === 'completado' || n.estado === 'omitido'; }).length;
    ...
    if(container){
      ...
      nodes.forEach(function(n, idx){
        ...
        var label = document.createElement('div'); label.className = 'node-label'; label.textContent = (idx + 1) + '. ' + n.tipo + (n.routeLabel ? ' · ' + n.routeLabel : '');
  ...
  async function resolverNodoEspecialMP(idx, accion){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        if(window.completarNodoMapa) window.completarNodoMapa(mapNodes,idx);
        else { mapNodes[idx].estado='completado'; if(mapNodes[idx+1]) mapNodes[idx+1].estado='disponible'; }
  ...
  function generarEnemigosParaNodoMP(node, totalNodos){
    ...
    for(var i=0;i<cantidad;i++){
      ...
      enemigos.push({
        ...
        patternIndex: 0,
        archetype: base.archetype || '', trait: base.trait || '', phases: base.phases || [], statusResistances:(base.statusResistances||[]).slice(), phaseIndex:0, phaseName:'',
        rageBonus:0, chargedBonus:0, counter:0, objective:base.objective || null,
        elite: !!base.elite,
        boss: !!base.boss
      });
    }
  ...
  function rollDiceForCombatPlayer(p, round){
    // Regla central: ronda 1 = 1 dado, ronda 2 = 2... hasta 6.
    var count = window.CardEngine
      ? window.CardEngine.getDiceCountForRound(round || 1, p)
      : Math.max(1, Math.min(6, round || 1));
    p.diceCount = count;
    var dice = [];
  ...
  // Sincronía cooperativa: premia alternar acciones distintas entre jugadores.
  function etiquetaAccionCoop(carta){
    if(!carta) return '🥊';
    var tipo=(carta.tipoCatalogo||carta.clase||carta.tipoCarta||'').toString();
    if(tipo==='Defensa'||carta.tipoEfecto==='defensa') return '🛡️';
    if(tipo==='Recuperación'||tipo==='Cura'||carta.tipoEfecto==='cura') return '❤️';
    if(carta.naturaleza==='Ki'||tipo.indexOf('Técnica')!==-1) return '⚡';
    if(carta.naturaleza==='Físico'||carta.tipoEfecto==='ataque') return '🥊';
    if(tipo==='Combo') return '🔥';
    return '🌀';
  }

  function actualizarSincroniaEquipo(combat, uid, carta, logArr){
    if(!combat || !combat.players) return;
    var sync=combat.teamSync || {value:0,lastPlayer:null,lastTag:null};
    var tag=etiquetaAccionCoop(carta);
    if(!sync.lastPlayer){
      sync.value=1;
    } else if(sync.lastPlayer!==uid && sync.lastTag!==tag){
      sync.value=Math.min(3,(sync.value||0)+1);
    } else if(sync.lastPlayer!==uid){
      sync.value=Math.max(1,(sync.value||0));
    } else {
      // Repetir jugador corta la cadena, pero no la elimina del todo.
      sync.value=Math.max(0,(sync.value||0)-1);
    }
    sync.lastPlayer=uid; sync.lastTag=tag;
    if(sync.value>=3){
      Object.keys(combat.players).forEach(function(pid){
        var pl=combat.players[pid]; if(!pl || pl.health<=0) return;
        pl.energy=(pl.energy||0)+1;
        if(window.Engagement) window.Engagement.addAwakening(pl,3,crearContextoCartaMP(pl,combat,logArr,pid,0),'sincronía');
      });
      logArr.push(entry('🤝 ¡Sincronía de equipo! Ambos jugadores recuperan 1 de energía y ganan +3 de Despertar.'));
      sync.value=0; sync.lastPlayer=null; sync.lastTag=null;
    }
    combat.teamSync=sync;
  }

  function crearContextoCartaMP(p, combat, logArr, uid, paidCost){
    var target = (combat.enemies || []).find(function(e){ return e.id === state.selectedEnemyId && e.health > 0; })
    ...
    return {
      ...
      playerId: uid,
      partner: partner,
      allPlayers: combat.players,
      target: target,
      targets: (combat.enemies || []).filter(function(e){ return e.health > 0; }),
      host: combat,
      round: combat.round || 1,
      paidCost: paidCost || 0,
      log: function(msg){ logArr.push(entry(msg)); },
      onKill: function(t){
        if(window.CardEngine && t && t.boss){
          window.CardEngine.runtime(p).once.bossKilled = true;
        }
      },
      damageOthers: function(amount, primaryTarget, attacker){
        (combat.enemies || []).forEach(function(e){
          if(e !== primaryTarget && e.health > 0) e.health = Math.max(0, e.health - (amount || 0));
        });
        Object.keys(combat.players || {}).forEach(function(pid){
          var pl = combat.players[pid];
          if(pl && pl !== attacker && pl.health > 0) pl.health = Math.max(0, pl.health - (amount || 0));
        });
      }
    };
  }
  ...
  async function iniciarCombateMP(idx){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        if(node.branchGroup && window.seleccionarRamaMapa) window.seleccionarRamaMapa(room.mapNodes,idx);
        node.estado = 'in-progress';

        var enemigos = generarEnemigosParaNodoMP(node, room.mapNodes.length);
        if((node.acto||1)===3) enemigos.forEach(function(e){e.rageBonus=(e.rageBonus||0)+1;});
        ...
        var combat = {
          ...
          engagementObjective: enemigos[0] && enemigos[0].objective ? JSON.parse(JSON.stringify(enemigos[0].objective)) : null,
          sagaAct: node.acto || 1,
          teamSync: { value:0, lastPlayer:null, lastTag:null }
        };
        if(window.Engagement && combat.engagementObjective) combat.engagementObjective.progress = combat.engagementObjective.progress || {};
        Object.keys(combatPlayers).forEach(function(pid){
          var cp=combatPlayers[pid]; if(window.Engagement) window.Engagement.resetCombat(cp);
          if(window.CardEngine) window.CardEngine.onTurnStart(cp, crearContextoCartaMP(cp,combat,combat.log,pid,0));
        });
        tx.update(roomRef, { status:'combat', currentNodeIndex:idx, mapNodes:mapNodes, combat:combat, updatedAt:serverTimestamp() });
      });
    } catch(err){ console.error(err); }
  }
  ...
  function finalizarCombateEnTransaccion(tx, roomRef, room, combat, victoria){
    ...
    uids.forEach(function(u){
      ...
      if(victoria){
        ...
        if((combat.sagaAct||1)===3) recompensa+=10;
    ...
    if(victoria){
      ...
      if(window.Engagement){
        var firstUid=uids[0], firstP=firstUid?combat.players[firstUid]:null;
        var obj=window.Engagement.objectiveStatus(combat,firstP,combat.round,true);
        if(obj && obj.done){
          uids.forEach(function(u){ room.players[u].gold=(room.players[u].gold||0)+obj.rewardGold; });
          combat.log=(combat.log||[]).concat([entry('🎯 Objetivo cooperativo completado: +'+obj.rewardGold+' de oro para cada jugador.')]);
        }
      }
    } else {
  ...
  async function jugarCartaMP(handIndex){
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        var log = combat.log || [];
        var ctx = crearContextoCartaMP(p, combat, log, uid, 0);
        var costeReal = carta.costo;
        if(window.CardEngine){
          var puede = window.CardEngine.canPlay(carta, p, ctx.target, ctx);
          if(!puede.ok){
            log.push(entry(carta.nombre + ' — ' + (puede.reason || 'condición no cumplida') + '.'));
            combat.log = log.slice(-40);
            tx.update(roomRef, { combat: combat, updatedAt: serverTimestamp() });
            return;
          }
          costeReal = window.CardEngine.getEffectiveCost(carta, p, ctx);
        }
        if((p.energy || 0) < costeReal) return;
        ...
        var targetBefore=ctx.target, hpBefore=targetBefore ? targetBefore.health : 0;
        if(window.CardEngine && carta.importada){
          ...
          var dealt=targetBefore?Math.max(0,hpBefore-targetBefore.health):0;
          if(window.CardEngine && window.CardEngine.recordExternalPlay) window.CardEngine.recordExternalPlay(carta,p,costeReal,dealt,ctx);
          if(window.Engagement) window.Engagement.afterCardPlayed(carta,p,ctx,dealt);
        }
        if(window.Engagement) (combat.enemies||[]).forEach(function(e){window.Engagement.checkEnemyPhase(e,ctx);});
        actualizarSincroniaEquipo(combat, uid, carta, log);
  ...
  function aplicarEfectoCartaMP(carta, p, combat, log){
    ...
    if(tipo === 'ataque'){
      ...
      if(objetivo){
        ...
        if(window.Engagement){var ctxMod=crearContextoCartaMP(p,combat,log,miUid(),carta.costo);ctxMod.target=objetivo;dmg=window.Engagement.modifyOutgoingDamage(p,objetivo,carta,dmg,ctxMod);}
      ...
      var antes = p.health;
      var healValue=window.Engagement?window.Engagement.modifyHeal(p,valor,crearContextoCartaMP(p,combat,log,miUid(),carta.costo),carta):valor;
      p.health = Math.min(p.maxHealth, p.health + healValue);
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        if(objetivo){
          ...
          var basicCtxMP=crearContextoCartaMP(p,combat,log,uid,0); basicCtxMP.target=objetivo;
          if(window.Engagement)dmg=window.Engagement.modifyOutgoingDamage(p,objetivo,{nombre:'Golpe Básico',tipoCatalogo:'Ataque',clase:'Ataque',naturaleza:'Físico',alcance:'Cercanía'},dmg,basicCtxMP);
          if(objetivo.shield > 0){
            var bloqueado = Math.min(objetivo.shield, dmg);
            dmg = Math.max(0, dmg - objetivo.shield);
            objetivo.shield = Math.max(0, objetivo.shield - bloqueado);
          ...
          var hpAntesBasico=objetivo.health;
          objetivo.health = Math.max(0, objetivo.health - dmg);
          var realBasico=Math.max(0,hpAntesBasico-objetivo.health);
          log.push(entry(p.name + ' lanza un Golpe Básico — inflige ' + realBasico + ' daño a ' + objetivo.name + '.'));
          if(window.Engagement){ window.Engagement.afterBasicAttack(p,basicCtxMP,realBasico); window.Engagement.checkEnemyPhase(objetivo,basicCtxMP); }
          actualizarSincroniaEquipo(combat, uid, {nombre:'Golpe Básico',tipoCatalogo:'Ataque',naturaleza:'Físico'}, log);
        }
        combat.log = log.slice(-40);
  ...
  async function activarDespertarMP(){
    if(!window.Engagement || !state.roomCode) return;
    var roomRef=doc(db,'rooms',state.roomCode), uid=miUid();
    try{
      await runTransaction(db,async function(tx){
        var snap=await tx.get(roomRef), room=snap.data(), combat=room.combat; if(!combat||!combat.players[uid])return;
        var p=combat.players[uid], log=combat.log||[], ctx=crearContextoCartaMP(p,combat,log,uid,0);
        var res=window.Engagement.activateAwakening(p,ctx); if(!res.ok)log.push(entry(res.reason||'Despertar no disponible.'));
        combat.players[uid]=p; combat.log=log.slice(-40); tx.update(roomRef,{combat:combat,updatedAt:serverTimestamp()});
      });
    }catch(err){console.error(err);}
  }

  async function compartirKiMP(){
    if(!state.roomCode) return;
    var roomRef=doc(db,'rooms',state.roomCode), uid=miUid();
    try{
      await runTransaction(db,async function(tx){
        var snap=await tx.get(roomRef), room=snap.data(), combat=room.combat;
        if(!combat || !combat.players || !combat.players[uid]) return;
        var p=combat.players[uid];
        var partnerUid=Object.keys(combat.players).filter(function(u){return u!==uid;})[0];
        var partner=partnerUid?combat.players[partnerUid]:null;
        if(!partner || partner.health<=0 || p.health<=0 || p.endedTurn) return;
        if((p.energy||0)<2 || p.sharedKiRound===combat.round) return;
        p.energy-=2; partner.energy=(partner.energy||0)+2; p.sharedKiRound=combat.round;
        var log=combat.log||[]; log.push(entry('⚡ '+p.name+' comparte 2 de Ki con '+partner.name+'.'));
        combat.players[uid]=p; combat.players[partnerUid]=partner; combat.log=log.slice(-40);
        tx.update(roomRef,{combat:combat,updatedAt:serverTimestamp()});
      });
    }catch(err){console.error(err);}
  }
    ...
    try{
      await runTransaction(db, async function(tx){
        ...
        combat.enemies.forEach(function(enemy){
          ...
          var objetivoUid=vivos[0], objetivo=objetivoUid?combat.players[objetivoUid]:null;
          function enemyHit(breakGuard){
            if(!objetivo)return; var dmg=(mov.valor||0)+(enemy.rageBonus||0)+(enemy.chargedBonus||0); enemy.chargedBonus=0;
            var ctxIncoming=crearContextoCartaMP(objetivo,combat,log,objetivoUid,0);
            if(window.CardEngine){var mitig=window.CardEngine.beforeIncomingDamage(objetivo,enemy,dmg,ctxIncoming);dmg=mitig.damage;}
            if(objetivo.shield>0&&dmg>0){var usable=breakGuard?Math.ceil(objetivo.shield*.5):objetivo.shield;var bloqueado=Math.min(usable,dmg);dmg=Math.max(0,dmg-bloqueado);objetivo.shield=Math.max(0,objetivo.shield-bloqueado);if(bloqueado>0)log.push(entry(objetivo.name+' bloqueó '+bloqueado+' de daño.'));}
            var hpAntes=objetivo.health; objetivo.health=Math.max(0,objetivo.health-dmg); var real=Math.max(0,hpAntes-objetivo.health);
            if(window.CardEngine)window.CardEngine.afterIncomingDamage(objetivo,enemy,real,ctxIncoming);
            log.push(entry(enemy.name+(breakGuard?' usa Rompeguardia':' ataca a '+objetivo.name)+' — inflige '+real+' daño.'));
          }
          if(mov.tipo==='ataque') enemyHit(false);
          else if(mov.tipo==='rompeguardia') enemyHit(true);
          else if(mov.tipo==='bloqueo'){enemy.shield=(enemy.shield||0)+(mov.valor||0);log.push(entry(enemy.name+' se protege y gana '+(mov.valor||0)+' de escudo.'));}
          else if(mov.tipo==='cura'){var antes=enemy.health;enemy.health=Math.min(enemy.maxHealth,enemy.health+(mov.valor||0));log.push(entry(enemy.name+' se regenera '+(enemy.health-antes)+' HP.'));}
          else if(mov.tipo==='carga'){enemy.chargedBonus=(enemy.chargedBonus||0)+(mov.valor||0);log.push(entry(enemy.name+' carga Ki: próximo ataque +'+(mov.valor||0)+'.'));}
          else if(mov.tipo==='drenaje'&&objetivo){var dr=Math.min(objetivo.energy||0,mov.valor||0);objetivo.energy=Math.max(0,(objetivo.energy||0)-dr);log.push(entry(enemy.name+' drena '+dr+' de energía a '+objetivo.name+'.'));}
          else if(mov.tipo==='estado'&&objetivo){var ctxS=crearContextoCartaMP(objetivo,combat,log,objetivoUid,0);if(window.CardEngine)window.CardEngine.addStatus(objetivo,mov.estado||'Quemado',ctxS);}
          else if(mov.tipo==='furia'){enemy.rageBonus=(enemy.rageBonus||0)+(mov.valor||0);log.push(entry(enemy.name+' entra en Furia: +'+(mov.valor||0)+' daño a sus ataques.'));}
          else if(mov.tipo==='contra'){enemy.counter=mov.valor||0;log.push(entry(enemy.name+' prepara un contraataque de '+(mov.valor||0)+'.'));}
        ...
        uids.forEach(function(u){
          ...
          if(window.CardEngine && p){
            window.CardEngine.onTurnEnd(p, crearContextoCartaMP(p, combat, log, u, 0));
          }
        });
        ...
        uids.forEach(function(u){
          ...
          rollDiceForCombatPlayer(p, combat.round);
          if(window.CardEngine){
            window.CardEngine.onTurnStart(p, crearContextoCartaMP(p, combat, log, u, 0));
          }
        });
  ...
  function renderCombateMP(room){
    ...
    if(enemyRow){
      ...
      combat.enemies.forEach(function(en){
        ...
        var intentInfo=window.Engagement?window.Engagement.intentInfo(intento):{icon:INTENT_ICON[intento.tipo]||'❔',label:String(intento.valor||'')};
        var enemyStatuses=(window.CardEngine&&window.CardEngine.runtime)?(window.CardEngine.runtime(en).statuses||[]).map(function(st){return st.name+(st.stacks>1?'×'+st.stacks:'');}):[];
        var resistanceText=(en.statusResistances||[]).length?'Resiste primero: '+en.statusResistances.join(', '):'';
        card.innerHTML =
          ...
          ((en.archetype||en.phaseName||enemyStatuses.length||resistanceText)?'<div class="mp-enemy-trait">'+[en.archetype,en.phaseName,enemyStatuses.length?'☣️ '+enemyStatuses.join(', '):'',resistanceText].filter(Boolean).join(' · ')+'</div>':'') +
          ...
          (en.health > 0 ? '<div class="mp-enemy-intent">' + intentInfo.icon + ' ' + intentInfo.label + '</div>' : '<div class="mp-enemy-intent">💀 Derrotado</div>');
        if(en.health > 0){
    ...
    if(alliesRow){
      ...
      [ [uid, me, true], [partnerUid, partner, false] ].forEach(function(pair){
        ...
        card.innerHTML =
          ...
          '</div>' +
          '<div class="mp-ally-status">' + (p.health <= 0 ? '💀 Caído' : (p.endedTurn ? '✅ Esperando' : '⏳ En turno')) + (((window.CardEngine&&window.CardEngine.runtime)?(window.CardEngine.runtime(p).statuses||[]):[]).length ? ' · ☣️ '+window.CardEngine.runtime(p).statuses.map(function(st){return st.name+(st.stacks>1?'×'+st.stacks:'');}).join(', ') : '') + '</div>';
    ...
    if(handEl && me){
      ...
      (me.hand || []).forEach(function(carta, idx){
        ...
        if(carta.rareza) c.classList.add('rarity-' + carta.rareza.toLowerCase());
        var ctxPreview = crearContextoCartaMP(me, combat, combat.log || [], uid, 0);
        var costeReal = window.CardEngine
          ? window.CardEngine.getEffectiveCost(carta, me, ctxPreview)
          : carta.costo;
        var validacion = window.CardEngine
          ? window.CardEngine.canPlay(carta, me, ctxPreview.target, ctxPreview)
          : {ok:true};
        if(!puedoJugar || (me.energy||0) < costeReal || !validacion.ok) c.classList.add('no-energy');
        if(!validacion.ok) c.title = validacion.reason || 'Condición no cumplida';
        var metaParts = [];
        if(window.CardBalance) metaParts.push(window.CardBalance.formatRarity(carta));
        metaParts.push(carta.clase || carta.tipoCarta || '');
        if(carta.naturaleza) metaParts.push(carta.naturaleza);
        if(carta.alcance) metaParts.push(carta.alcance);
        if(carta.rondaMinima && carta.rondaMinima > 1) metaParts.push('R' + carta.rondaMinima + '+');
        var hint = !validacion.ok ? '<div class="card-rule-hint">' + (validacion.reason || 'Condición no cumplida') + '</div>' : '';
        c.innerHTML =
          '<div class="title">' + carta.nombre + '</div>' +
          '<div class="meta">' + metaParts.filter(Boolean).join(' · ') + '</div>' +
          '<div class="desc">' + (carta.efecto || '') + hint + '</div>' +
          '<div class="cost-row"><span class="cost-pip">' + costeReal + '</span></div>';
        if(puedoJugar){
    ...
    if(diceEl && me){
      diceEl.innerHTML = '';
      var vals=me.dice||[], counts={}; vals.forEach(function(v){counts[v]=(counts[v]||0)+1;});
      vals.forEach(function(v){
        var d = document.createElement('div'); d.className = 'die'+((counts[v]||0)>=2?' resonant':''); d.textContent = v;
    ...
    if(me && window.Engagement){
      var hud=window.Engagement.getHud(me,combat,combat.round);
      var flow=document.getElementById('mpFlowIndicator'); if(flow)flow.textContent='🔥 Flujo '+hud.flow+(hud.flowChain?' · '+hud.flowChain:'');
      var an=document.getElementById('mpAwakeningName'); if(an)an.textContent=hud.awakened?hud.awakeningName+' · '+hud.awakeningTurns+'T':hud.awakeningName;
      var av=document.getElementById('mpAwakeningValue'); if(av)av.textContent=hud.awakened?'ACTIVO':hud.awakening+'%';
      var af=document.getElementById('mpAwakeningFill'); if(af)af.style.width=(hud.awakened?100:hud.awakening)+'%';
      var ab=document.getElementById('mpAwakeningBtn'); if(ab){ab.disabled=me.endedTurn||hud.awakened||hud.awakening<100;ab.textContent=hud.awakened?'✨ '+hud.awakeningTurns+'T':'✨ Despertar';}
      var sagaEl=document.getElementById('mpCombatSaga'); if(sagaEl){var sg=window.Engagement.getSaga(combat.sagaAct||1);sagaEl.textContent=sg.icon+' '+sg.name+' — '+sg.desc;}
      var oi=document.getElementById('mpCombatObjective'); var os=window.Engagement.objectiveStatus(combat,me,combat.round,false); if(oi)oi.textContent=os?'🎯 '+os.text:'🎯 Combate libre';
      var syncEl=document.getElementById('mpTeamSync'); if(syncEl){var sv=(combat.teamSync&&combat.teamSync.value)||0;syncEl.textContent='🤝 Sincronía '+sv+'/3';syncEl.classList.toggle('is-hot',sv>=2);}
      var di=document.getElementById('mpDiceInsight'); var es=window.Engagement.state(me); if(di)di.textContent=es.resonance?'🎲 Resonancia: '+es.resonance:'🎲 Busca parejas o escaleras para obtener bonificaciones';
    }
    ...
    if(basicBtn && me){
      var puedeJugarAlguna = (me.hand||[]).some(function(c){
        var ctx = crearContextoCartaMP(me, combat, combat.log || [], uid, 0);
        var coste = window.CardEngine ? window.CardEngine.getEffectiveCost(c, me, ctx) : c.costo;
        var val = window.CardEngine ? window.CardEngine.canPlay(c, me, ctx.target, ctx) : {ok:true};
        return val.ok && (me.energy||0) >= coste;
      });
    ...
    var shareBtn=document.getElementById('mpShareKiBtn');
    if(shareBtn && me){
      var canShare=partner && partner.health>0 && !me.endedTurn && me.health>0 && (me.energy||0)>=2 && me.sharedKiRound!==combat.round;
      shareBtn.disabled=!canShare;
      shareBtn.textContent=canShare?'⚡ Compartir 2 Ki':'⚡ Ki compartido';
    }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    var awakeBtn = document.getElementById('mpAwakeningBtn');
    if(awakeBtn) awakeBtn.addEventListener('click', activarDespertarMP);
    var shareKiBtn = document.getElementById('mpShareKiBtn');
    if(shareKiBtn) shareKiBtn.addEventListener('click', compartirKiMP);
