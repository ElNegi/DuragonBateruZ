...
(function(){
  ...
  Game.prototype.showScreen = function(name){
    ...
    // Oculta también las pantallas cooperativas. Así cualquier transición
    // hacia Login/Menú/Expedición deja una única pantalla visible.
    var allScreenIds = [
  ...
  Game.prototype.resetJuego = function(targetScreen){
    ...
    // Conserva la identidad de la sesión Firebase al reiniciar una
    // expedición. Antes se conservaba solo el nombre y se perdía `uid`, lo
    // que impedía entrar a multijugador después de una derrota.
    var sesion = window.CURRENT_USER;
    if(targetScreen === 'login' || !sesion){
      window.CURRENT_USER = null;
    } else {
      window.CURRENT_USER = {
        uid: sesion.uid || null,
        name: sesion.name || 'Jugador',
        email: sesion.email || null,
        isAnonymous: !!sesion.isAnonymous,
        firebaseAuthenticated: !!sesion.firebaseAuthenticated,
        isLocalTest: !!sesion.isLocalTest
      };
    }
